...
"""
Common modules
定义常见的结构，以及封装前处理+推理+后处理。但是autoshape在dataset.py中
"""
...
from models.activations import *  # 导入激活函数
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        # group > 1 深度可卷积分离
        # act如果是true就是定义的这个激活函数，否则就是nn.module传入的激活函数
        super().__init__()
    ...
    def forward_fuse(self, x):
        """
        用于Model的fuse函数。融合conv+bn加速推理，一般用于验证、测试阶段
        """
...
class C3(nn.Module):  # 简化版本Bottleneckcsp
    # CSP Bottleneck with 3 convolutions
    """
    在C3TR模块中和yolo.py的parse_model调用
    """
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
...
# 空间+通道，双管齐下
class SCse(nn.Module):  # 这里就是个channel
...
# C3SE使用的是reduction自己定义就好
# 只用一个leakyrelu，插在3*3里面
...
class C3SE(nn.Module):  # 简化版本Bottleneckcsp
    # CSP Bottleneck with 3 convolutions
    """
    在C3TR模块中和yolo.py的parse_model调用
    """
...
class C3CA(nn.Module):  # 简化版本Bottleneckcsp
    # CSP Bottleneck with 3 convolutions
    """
    在C3TR模块中和yolo.py的parse_model调用
    """
...
class BiConcat(nn.Module):
    ...
    def _forward(self, x):
        if len(x) == 2:
            ...
            w = self.w2
            weight = w / (torch.sum(w, dim=0) + self.epsilon)
            # (1, 128, 16, 16)  第6层的channel(1, 256, 16, 16)  (1, 128, 16, 16)
        ...
        #     还是尺寸不匹配的问题....分析一下所有的batch-size去看看
        return x
...
class DetectMultiBackend(nn.Module):
    ...
    # warmup学习率优化算法：训练初期用小的学习率，一段时间后使用预设的学习率训练。
...
class AutoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        # 如果是tensor格式，直接向前推理即可。nms在推理结束后进行。
...
# 二级分类检测。对于车牌识别：先识别出车牌，再对车牌上的字检测
